...
for di = 1:length(data_L2_list)
    ...
    eval([' data_L2 = load_data.RS_L2_',data_L2_list(di).name(10:end-4),'; ']);
    ...
    tmp_data.SondeID = data_L2.SondeID;
    ...
    if ( exist([L4_output_file_path,data_L2_list(di).name],'file') ~= 0 )
        ...
        tmp_data.Ascent = interp1(tmp_p,data_L2.Ascent(~nan_p),tmp_data.P,'linear');
        ...
        eval([ ['RS_L4u_',data_L2_list(di).name(10:19)],' = tmp_data;' ]);
        
        %% Output the data:
        
        L4_output_file_name = [L4_output_file_path,data_L2_list(di).name];
        
        save(L4_output_file_name,'RS_L4u*');
