...
% Transfer each data from L3 (Aspen-QCed, mat format) to L4 (mat format).
% 
% ==============================================================================
...
%% File path:
L3_input_file_path     = ['../L3_TASSE_mat/',year_no{1},'/'];
L4_output_file_path    = ['./',year_no{1},'/'];

data_L3_list = dir([L3_input_file_path,'*.mat']);
        
for di = 1:length(data_L3_list)
    
    load_data = load([L3_input_file_path,data_L3_list(di).name]);
    
    eval([' data_L3 = load_data.ST_L3_',data_L3_list(di).name(10:end-4),'; ']);
    
    clear load_data
    
    %% Put data in the output format:
    
    % tmp_data.Station = data_L3.Station;
    
    tmp_data.NominalTime = data_L3.NominalTime;
    
    tmp_data.ST_No = data_L3.ST_No;
    
    tmp_data.LaunchLocation = data_L3.LaunchLocation;
    
    tmp_data.LaunchTime = data_L3.LaunchTime;
    
    if ( exist([L4_output_file_path,data_L3_list(di).name],'file') ~= 0 )
        
        disp([data_L3_list(di).name,' already done.'])
        
    else
        
        fprintf(fileID,'%s\n',data_L3_list(di).name);
        
        %% Interpolation to P:
        
        if ( mod(data_L3.P(1),p_res) ~= 0 )
            tmp_data.P = [data_L3.P(1),floor(data_L3.P(1)./p_res)*p_res:-p_res:ceil(min(data_L3.P)./p_res)*p_res]';
        else
            tmp_data.P = [data_L3.P(1),floor(data_L3.P(2)./p_res)*p_res:-p_res:ceil(min(data_L3.P)./p_res)*p_res]';
        end
        
        nan_p = isnan(data_L3.P);
        tmp_p = data_L3.P(~nan_p) + [1:numel(data_L3.P(~nan_p))]'.*1e-7; % ADD AN SMALL MONOTONIC EPSILON TO ENSURE THE INTERPOLATION.
        
        tmp_data.TIME_s = interp1(tmp_p,data_L3.TIME_SEC(~nan_p),tmp_data.P,'nearest');
        % tmp_data.Z      = interp1(tmp_p,data_L3.Z(~nan_p),tmp_data.P,'linear');
        tmp_data.Z_GPS  = interp1(tmp_p,data_L3.Z_GPS(~nan_p),tmp_data.P,'linear');
        tmp_data.TC     = interp1(tmp_p,data_L3.TC(~nan_p),tmp_data.P,'linear');
        tmp_data.TD     = interp1(tmp_p,data_L3.TD(~nan_p),tmp_data.P,'linear');
        tmp_data.RH     = interp1(tmp_p,data_L3.RH(~nan_p),tmp_data.P,'linear');
        tmp_data.RHI    = interp1(tmp_p,data_L3.RHI(~nan_p),tmp_data.P,'linear');
        tmp_data.WD     = interp1(tmp_p,data_L3.WD(~nan_p),tmp_data.P,'nearest');
        tmp_data.WS     = interp1(tmp_p,data_L3.WS(~nan_p),tmp_data.P,'linear');
        tmp_data.U      = interp1(tmp_p,data_L3.U(~nan_p),tmp_data.P,'linear');
        tmp_data.V      = interp1(tmp_p,data_L3.V(~nan_p),tmp_data.P,'linear');
        
        % Geopotential Height:
        for tmpi = 1:length(tmp_data.P)
            if ( tmpi == 1 )
                tmp_data.Z(tmpi) = data_L3.LaunchLocation(3);
            else
        ...
        data_L3.LON(data_L3.LON==-999) = NaN;
        data_L3.LAT(data_L3.LAT==-999) = NaN;
        tmp_data.LON    = interp1(tmp_p,data_L3.LON(~nan_p),tmp_data.P,'linear');
        tmp_data.LAT    = interp1(tmp_p,data_L3.LAT(~nan_p),tmp_data.P,'linear');
        ...
        fprintf(fileID,'%s\n',[data_L3_list(di).name,' done.']);
        
        eval([ ['ST_L4c_',data_L3_list(di).name(10:19)],' = tmp_data;' ]);
        
        %% Output the data:
        
        L4_output_file_name = [L4_output_file_path,data_L3_list(di).name];
        
        save(L4_output_file_name,'ST_L4c*');
        
        fprintf(fileID,'%s\n',[L4_output_file_path,data_L3_list(di).name,' data saved.']);
        
        disp([data_L3_list(di).name,' data saved.'])
